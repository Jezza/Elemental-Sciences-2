...
public class PacketPipeline extends MessageToMessageCodec<FMLProxyPacket, IPacket> {
    ...
    public boolean registerPacket(Class<? extends IPacket> clazz) {
        if (this.packets.size() > 256) {
            ESLogger.severe("Packet size exceeded!");
            return false;
        }

        if (this.packets.contains(clazz)) {
            ESLogger.info("Packet is already registered!");
            return false;
        }

        if (this.isPostInitialised) {
            ESLogger.severe("Cannot register due to already initalised!");
            return false;
        }
    ...
    protected void decode(ChannelHandlerContext ctx, FMLProxyPacket msg, List<Object> out) throws Exception {
        ...
        pkt.readBytes(ctx, payload.slice());

        EntityPlayer player;
        switch (FMLCommonHandler.instance().getEffectiveSide()) {
            case CLIENT:
                player = this.getClientPlayer();
                pkt.executeClientSide(player);
                break;

            case SERVER:
                INetHandler netHandler = ctx.channel().attr(NetworkRegistry.NET_HANDLER).get();
                player = ((NetHandlerPlayServer) netHandler).playerEntity;
                pkt.executeServerSide(player);
                break;
