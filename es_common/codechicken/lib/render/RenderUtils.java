...
public class RenderUtils {
	...
	/**
	 * Renders a fluid within a bounding box. If the fluid is a liquid it will render as a normal tank with height equal to density/bound.height. If the fluid is a gas, it will render the full box with an alpha equal to density. Warning, bound will be mutated if the fluid is a liquid
	 * 
	 * @param stack
	 *            The fluid to render.
	 * @param bound
	...
	public static void renderFluidGauge(FluidStack stack, Rectangle4i rect, double density, double res) {
		...
		Icon tex = prepareFluidRender(stack, alpha);
		CCRenderState.startDrawing(7);
		renderFluidQuad(new Vector3(rect.x, rect.y, 0), new Vector3(rect.x, rect.y + rect.h, 0), new Vector3(rect.x + rect.w, rect.y + rect.h, 0), new Vector3(rect.x + rect.w, rect.y, 0), tex, res);
		CCRenderState.draw();
		postFluidRender();
	}
